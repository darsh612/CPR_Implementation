...
void pipeline_t::retire(size_t& instret, size_t instret_limit) {
   ...
   while(true){
     switch(RETSTATE.state){
      case retire_state_e::RETIRE_IDLE:
      ...
      case retire_state_e::RETIRE_BULK_COMMIT:
         RETSTATE.log_reg = 0;
         for(unsigned int x=0;x<RETIRE_WIDTH;i++){
         ...
         if((RETSTATE.num_loads_left == 0) && (RETSTATE.num_stores_left == 0) && (num_stores_left == 0) && (RETSTATE.log_reg == NXPR+NFPR)){
            REN->free_checkpoint();
            RETSTATE.state = retire_state_e::RETIRE_FINALIZE;
         }
      case retire_state_e:: RETIRE_FINALIZE:
